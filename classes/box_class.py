...
class majorana_box:
    def __init__(self, majoranas, overlaps, Vg=0, name='Unidentified'):
        ...
        self.par = np.concatenate(
            (np.zeros(2**(self.elec_num - 1)),
             np.ones(2**(self.elec_num - 1)))).astype(int)
    ...
    def diagonalize(self):
        self.energies, self.U = fbr.rotated_system(self.elec_num,
                                                   self.overlaps)
        self.adj_charging(self.Vg)
        self.diagonal = True
    ...
    def box_via_edges(cls, list_of_edges, Vg=0):
        majoranas = np.concatenate(
            [np.array(edge.majoranas) for edge in list_of_edges])
        number_majoranas = np.max([maj.index for maj in majoranas]) + 1

        if number_majoranas != majoranas.size:
            print(
                'Indices of Majoranas invalid! Can not build a box with that input.'
            )
            return
        ...
        for maj in majoranas:
            for partner_ind in maj.overlaps:
                energy_overlaps[partner_ind,
                                maj.index] += maj.overlaps[partner_ind]
        ...
        if self.diagonal:
            for k, col in enumerate(self.U.transpose()):
                ...
                for l, elem in np.ndenumerate(column):
                    if np.abs(elem) > 0.01:
                        superpos += str(
                            self.states.get_state(l, fac=np.round(elem,
                                                                  3))) + ' + '
                superpos = superpos[:-2]
                print(superpos)
        else:
    ...
    def constr_tunnel(self):
        ...
        if self.diagonal:
            self.tunnel = np.array(
                [np.dot(self.U.getH(), np.dot(t, self.U)) for t in tunnel])
        else:
            print(
                'System not yet diagonalized. Tunnelmatrix in default basis returned!'
            )
            self.tunnel = tunnel
        return self.tunnel

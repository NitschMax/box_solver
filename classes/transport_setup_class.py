...
class transport_setup:
    ...
    def overlap_arr(self):
        return np.array([
            self.eps01, self.eps12, self.eps23, self.eps_abs_0, self.eps_abs_1,
            self.eps_abs_2, self.eps_abs_3
        ])
    ...
    def build_qmeq_sys(self):
        if not self.box_assigned or not self.box_connected or not self.leads_initialized:
            print(
                'Can not use builder of qmeq as no Majorana box assigned or connected'
            )
            return None
        else:
            if self.i_n:
                return Builder_many_body(Ea=self.Ea,
                                         Na=self.par,
                                         Tba=self.tunnel,
                                         dband=self.dband,
                                         mulst=self.mu_lst,
                                         tlst=self.T_lst,
                                         kerntype=self.method,
                                         itype=self.itype,
                                         countingleads=self.counting_leads)
            else:
                return Builder_many_body(Ea=self.Ea,
                                         Na=self.par,
                                         Tba=self.tunnel,
    ...
    def disconnect_from_source(self):
        if self.box_symmetry == 2:
            ...
            print(
                'Disconnection of source for this geometry not yet implemented'
            )
        self.initialize_box()

    def adjust_to_z_blockade(self, gamma=1.0, phase_angle_guess=np.pi / 2):
        if self.box_symmetry == 1:
            ...
            guess = [
                1.0, 1 / 2 * np.pi
            ]  ## Blockade-condition for z-blockade in a pure majorana box
            bounds = ((0, np.inf), (-np.pi, np.pi))
            result = minimize(opt_func, guess, bounds=bounds).fun
            ...
            self.gamma_00 = gamma
            self.phi1 = 0.0
            guess = np.array([gamma, gamma, phase_angle_guess])
            opt_func = lambda x: self.tune_couplings_to_z_blockade(x)
            bnds = ((0, np.inf), (0, np.inf), (0, 2 * np.pi))
            constraint = LinearConstraint([1, 1, 0], [1], [np.inf])
            result = minimize(opt_func,
                              guess,
                              tol=self.dphi**2,
    ...
    def tune_couplings_to_z_blockade(self, couplings):
        ...
        self.gamma_01 = couplings[1]
        self.phi0 = couplings[2]
        self.initialize_box()
        return self.maj_box.calculate_blockade_cond(lead=0)

    def adjust_to_x_blockade(self, gamma=1.0, phase_angle_guess=np.pi / 2):
        if self.box_symmetry == 1:
            result = self.adjust_to_z_blockade(gamma)
        elif self.box_symmetry == 3:
            self.gamma_00 = 0.0
            self.gamma_01 = 0.0
            self.gamma_11 = gamma
    ...
    def tune_couplings_to_x_blockade(self, couplings):
        self.gamma_01 = couplings[0]
        self.gamma_02 = couplings[1]
        self.phi2 = couplings[2]
        self.initialize_box()
        return self.maj_box.calculate_blockade_cond(lead=0)
    ...
    def classify_edges(self, lead):
        potential_edges = 1 * np.array(
            [lead in edgy.cl for edgy in self.edges],
            dtype='bool')  ## Find out which edges are connected to lead
        return potential_edges

    def tune_phases(self, angles, lead):
        edge_classif = np.array([1, 0, 1, 0])
        edge_classif = self.classify_edges(lead)
        edge_phase_angles = np.array(
            [self.phi0, self.phi1, self.phi2, self.phi3])
        angles = edge_classif * angles + (
            1 - edge_classif
    ...
    def tune_rates(self, rates, lead, tuneable_rates):
        ...
        edge_rates = np.array(
            [self.gamma_00, self.gamma_01, self.gamma_02, self.gamma_e3])
        rates = edge_classif * rates + (
            1 - edge_classif
    ...
    def initialize_box(self):
        ...
        if self.model == 1:
            ...
            edgy0.create_majorana(0)
            edgy0.create_majorana(1,
                                  wf_factor=self.factor0,
                                  wf_phase_angle=self.th0,
                                  overlaps={0: self.eps_abs_0})

            edgy1.create_majorana(2, overlaps={0: 0 * self.eps01})
            edgy2.create_majorana(3, overlaps={2: 0 * self.eps12})

            edgy1.create_majorana(4,
                                  wf_factor=self.factor1,
                                  wf_phase_angle=self.th1,
                                  overlaps={2: self.eps_abs_1})
            edgy2.create_majorana(5,
                                  wf_factor=self.factor2,
                                  wf_phase_angle=self.th2,
                                  overlaps={3: self.eps_abs_2})

            edgy3.create_majorana(6, overlaps={3: 0 * self.eps23})
            edgy3.create_majorana(7,
                                  wf_factor=self.factor3,
                                  wf_phase_angle=self.th3,
                                  overlaps={6: self.eps_abs_3})
        elif self.model == 3:
            edgy0.create_majorana(0)
            edgy1.create_majorana(1, overlaps={0: self.eps01})
            edgy2.create_majorana(2, overlaps={1: self.eps12})
            edgy3.create_majorana(3, overlaps={2: self.eps23})
            edgy0.create_majorana(4,
                                  wf_factor=self.factor0,
                                  wf_phase_angle=self.th0,
                                  overlaps={
                                      0: self.eps_abs_0,

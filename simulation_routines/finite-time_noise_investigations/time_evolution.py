...
def main():
    ...
    if pre_run:
        t_set_z, t_set_x, sys_z, sys_x, rho0 = do_pre_run()
        t_set = t_set_z
    else:
        t_set = st.create_transport_setup()
    t_set = t_set_z
...
def do_pre_run():
    t_set_x = st.create_transport_setup()
    ...
    current_fct_z = current(sys_z, i_n=t_set_z.i_n)
    ...
    return t_set_z, t_set_x, sys_z, sys_x, rho0


def check_validity_of_EV(sys):
    largest_ev = quasi_stationary_states(sys, real=True, imag=True,
                                         number=1)[0].real
    print(largest_ev)
...
def the_code_does_work_plot(fig, ax, X, Y, t_set):
    ...
    for idx, dummy in np.ndenumerate(X):
        ...
        sys = t_set.build_qmeq_sys()
        sys.solve(qdq=False, rotateq=False)
        st_state_eigen[idx] = quasi_stationary_states(sys,
                                                      real=True,
                                                      imag=True,
                                                      number=1)[0].real
...
def tunnel_matrix_plot(axes, t_set):
    ...
    print('Largest eigenvalue of the Liouvillion',
          quasi_stationary_states(sys, real=True, imag=True, number=1)[0].real)
...
def eigenvalue_plot(ax, energies, t_set, number=0, real=True, imag=True):
    ...
    for eps in energies:
        ...
        eigenvalues.append(
            quasi_stationary_states(sys, real=real, imag=imag, number=number))
...
def filter_smallest_eigen(eigenval,
                          real=True,
                          imag=False,
                          order=True,
                          number=0):
...
def quasi_stationary_states(sys, real=True, imag=False, number=0):
    ...
    smallest_rate = filter_smallest_eigen(eigenval,
                                          real=real,
                                          imag=imag,
                                          number=number)
...
def charge_transmission(current_fct,
                        time_evo_rho,
                        rho0,
                        tzero=0,
                        tau=np.inf,
                        ...
                        use_nquad=False):
    if not use_nquad:
        result = quad(lambda x: current_fct(time_evo_rho(rho0, x))[0],
                      tzero,
                      tau,
                      epsrel=epsrel)
    else:
        opts = {'epsrel': epsrel}
        result = nquad(lambda x: current_fct(time_evo_rho(rho0, x))[0],
                       [(tzero, tau)],
                       opts=opts)
    return result
...
def calculate_finite_time_current(sys,
                                  rho0,
                                  t,
                                  lead=[0],
                                  plot_charge=True,
    ...
    if sys_2 is not None:
        ...
        for time in t:
            ...
            if plot_charge:
                ...
                charge_flow = charge_transmission(current_fct,
                                                  time_evo_rho,
                                                  rho0,
                                                  tzero=0,
                                                  tau=integration_cut,
                                                  epsrel=1.5e-8,
                                                  use_nquad=False)[0]
                # charge_flow += charge_transmission(current_fct_2,
                #                                    time_evo_rho_2,
                #                                    rho0_2,
                #                                    tzero=0,
                #                                    tau=integration_cut,
        ...
        finite_cur = np.array(
            [current_fct(time_evo_rho(rho0, time)) for time in t])
        charge = np.cumsum(finite_cur[:, lead] *
                           np.transpose([np.gradient(t)]))
    return finite_cur, charge


def finite_time_plot(ax,
                     ax_twin,
                     sys,
                     rho0,
                     t,
...
def partial_current(sys, lead=0, i_n=False):
    ...
    I_matrix_plus = np.block([[ones,
                               get_I_matrix(sys, 1, lead=lead) * ones],
                              [ones, ones]])
    I_matrix_minus = np.block([[ones,
                                get_I_matrix(sys, -1, lead=lead) * ones],
                               [ones, ones]])
    ...
    current_p = lambda ind: -2 * 2 * np.pi * np.trace(
        np.imag(
            np.dot(TbaLeft,
                   np.dot(map_vec_to_den_mat(sys, U_r[:, ind]), TbaRight))))
    return current_p
...
def current_self_implemented(sys, lead=0, i_n=False):
    ...
    current_rho = lambda rho: -2 * 2 * np.pi * np.trace(
        np.imag(np.dot(TbaLeft, np.dot(map_vec_to_den_mat(sys, rho), TbaRight))
                ))
    return current_rho
...
def princ_int(sys, lead=0, include_eigenenergies=1):
    ...
    x_L = get_x_from_sys(sys,
                         lead,
                         include_eigenenergies=include_eigenenergies)
    return np.real(digamma(0.5 + 1j * x_L /
                           (2 * np.pi))) - np.log(D / (2 * np.pi * T))
...
def get_x_from_sys(sys, lead=0, include_eigenenergies=1):
    ...
    for indices, value in np.ndenumerate(matrix_of_energydiffs):
        matrix_of_energydiffs[indices] = energies[indices[0]] - energies[
            indices[1] + par_occ]
    matrix_of_energydiffs *= include_eigenenergies
    x_L = (-matrix_of_energydiffs - sys.mulst[lead]) / sys.tlst[
        lead]  # Minus before energies because of indices cb for I compared to indices bc for Tba
    return x_L
...
def map_vec_to_den_mat(sys, rho):
    ...
    for indices, value in np.ndenumerate(even_par):
        ...
        if indices[0] < indices[1]:
            even_par[indices] += 1j * rho[
                sys.si.get_ind_dm0(indices[0], indices[1], 0) + half_ofd_dof]
    ...
    for indices, value in np.ndenumerate(odd_par):
        ...
        if indices[0] < indices[1]:
            odd_par[indices] += 1j * rho[
                sys.si.get_ind_dm0(indices[0], indices[1], 1) + half_ofd_dof]
        elif indices[0] > indices[1]:
            odd_par[indices] -= 1j * rho[
                sys.si.get_ind_dm0(indices[0], indices[1], 1) + half_ofd_dof]
...
def map_den_mat_to_vec(sys, rhoMat):
    ...
    for indices, values in np.ndenumerate(iteration_help):
        rho[sys.si.get_ind_dm0(indices[0], indices[1],
                               0)] = rhoMat[indices].real
        rho[sys.si.get_ind_dm0(indices[0], indices[1],
                               1)] = rhoMat[indices[0] + par_occ,
                                            indices[1] + par_occ].real
        if indices[0] < indices[1]:
            rho[sys.si.get_ind_dm0(indices[0], indices[1], 0) +
                half_ofd_dof] = rhoMat[indices].imag
            rho[sys.si.get_ind_dm0(indices[0], indices[1], 1) +
                half_ofd_dof] = rhoMat[indices[0] + par_occ,
                                       indices[1] + par_occ].imag
...
def stationary_state_limit(sys, rho0):
    ...
    zero_ind = np.argmin(np.abs(eigenval))
    smallest_rate, indices = filter_smallest_eigen(eigenval,
                                                   real=True,
                                                   imag=False)
...
def finite_time_evolution(sys, qs_desc=False):
    ...
    smallest_rate, small_indices = filter_smallest_eigen(eigenval,
                                                         real=True,
                                                         imag=False)
    time_evol_mats = np.array([
        np.dot(U_r[:, index],
               U_l.getH()[index]) for index in range(dimensions)
    ])
    ...
    time_evol = lambda rho0, t: normed_occupations(
        np.matmul(
            np.transpose(np.tensordot(time_evol_mats, rho0.copy(), axes=1)),
            np.exp(eigenval * t)))

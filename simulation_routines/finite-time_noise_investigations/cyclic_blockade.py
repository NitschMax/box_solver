...
def main():
	...
	charge_waiting_plot(ax, waiting_times, current_fct_z, current_fct_x, time_evo_rho_z, time_evo_rho_x, rho0, n, pre_run, sys_z, timescale=timescale, include_Irem=include_Irem)
	plt.show()
	return
...
def charge_angle_plot(ax, angles, t_set_z, t_set_x, n, pre_run, waiting_time, timescale=1, include_I_rem=False):
	charge	= []
	for angle in angles:
		t_set_z.th2	= angle
		t_set_x.th2	= angle
		...
		rho0, sys_z, sys_x, current_fct_z, current_fct_x, time_evo_rho_z, time_evo_rho_x	= initialize_cycle_fcts(t_set_z, t_set_x, qs_desc=False, initialization=2, lead=0)
		charge.append(charge_transmission_cycle(current_fct_z, current_fct_x, time_evo_rho_z, time_evo_rho_x, rho0, n, waiting_time, sys_z, pre_run=pre_run, include_Irem=include_Irem) )

	charge	= np.array(charge)
	ax.scatter(angles, charge, marker='x', label='Numverical results')
	angles		= np.linspace(0, np.pi/2-1e-2, 100)
	analytic_result	= np.array([2*qsi.analytical_prediction(angle ) for angle in angles ] )
...
def charge_waiting_plot(ax, waiting_times, current_fct_z, current_fct_x, time_evo_rho_z, time_evo_rho_x, rho0, n, pre_run, sys, timescale=1, include_Irem=False):
	#print('Neglected charge of the order', timescale**(-2)/2*waiting_times[-1])
	charge		= np.array([charge_transmission_cycle(current_fct_z, current_fct_x, time_evo_rho_z, time_evo_rho_x, rho0, n, waiting_time, sys, pre_run=pre_run, include_Irem=include_Irem) for waiting_time in waiting_times ] )
...
def charge_transmission_cycle(current_fct_1, current_fct_2, time_evo_1, time_evo_2, rho0, n, T, sys, pre_run=1e3, include_Irem=False):
	rho	= rho0
	...
	for i in range(n):
		...
		additional_charge	= te.charge_transmission(current_fct_1, time_evo_1, rho, tzero=0, tau=integration_range)[0]
		if include_Irem and T>integration_range:
			additional_charge	+= te.charge_transmission(current_fct_1, time_evo_1, rho, tzero=integration_range, tau=T, epsrel=epsrel)[0]
		charge_transmitted	+= additional_charge
		rho	= time_evo_1(rho, T)

		additional_charge	= te.charge_transmission(current_fct_2, time_evo_2, rho, tzero=0, tau=integration_range)[0]
		if include_Irem and T>integration_range:
			additional_charge	+= te.charge_transmission(current_fct_2, time_evo_2, rho, tzero=integration_range, tau=T, epsrel=epsrel)[0]
		charge_transmitted	+= additional_charge
		rho	= time_evo_2(rho, T)
